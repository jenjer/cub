...
/*   Updated: 2023/05/17 22:29:49 by gyopark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_line(t_press *press, double x1, double y1, double x2, double y2)
{
	...
	while (1)
	{
		...
		if (!check_wall_light(press, ray_x, ray_y))
		{
			...
			press->ray2->last_pre_x = ray_x;
			press->ray2->last_pre_y = ray_y;
		}
		else
		{
...
void	draw_one_ray(t_press *press, double angle, int ray_num)
{
	...
	press->ray_arr->ray_pre_x[ray_num] = press->ray2->last_pre_x;
	press->ray_arr->ray_pre_y[ray_num] = press->ray2->last_pre_y; 
}
...
void	ray_arr_init(t_ray_arr *ray_arr)
{
	...
	memset(ray_arr->distances, 0, sizeof(double) * RAY_COUNT);
	ray_arr->ray_pre_x = (double *)malloc(sizeof(double) * RAY_COUNT);
	memset(ray_arr->distances, 0, sizeof(double) * RAY_COUNT);
	ray_arr->ray_pre_y = (double *)malloc(sizeof(double) * RAY_COUNT);
	memset(ray_arr->distances, 0, sizeof(double) * RAY_COUNT);
